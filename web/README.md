...
后端每个响应都带 `requestId`（信封里一份，`X-Request-Id` 响应头里一份），失败时
还带一个稳定的 `error` 分类。`api/client.ts` 把这两样都挂到 `ApiError` 上，包括
根本没有信封的失败——代理返回的 502、截断的响应体——那时候从响应头取，而那正是最
需要这个 id 的场合。

怎么展示是 `lib/errors.ts` 一处说了算：

- `errorTitle(err)`——标题，用后端的 message（本来就是写给用户看的中文）
- `errorDetail(err)`——第二行，**只在用户无能为力时**给出日志 ID。重名了就改个名
  字，在那条消息底下挂个 uuid 只会训练人忽略 uuid；内部错误则相反，那个 id 是屏
  幕上唯一有用的东西
- `<ErrorState error={err} />`——加载失败的面板统一长这样，有 id 就显示

判断分支请用 `err.kind`（`conflict` / `permission_denied` / …），不要匹配
`message`。特别是 **403 不清登录态**：那是"你不能做这件事"，不是"重新登录"——
用户端账号打开一个管理端 URL 就会撞上 403，踢他去登录页是错的。
