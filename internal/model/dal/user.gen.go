...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.TrafficPlan = userHasOneTrafficPlan{
		...
		RelationField: field.NewRelation("TrafficPlan", "entity.TrafficPlan"),
	}
...
type userHasOneTrafficPlan struct {
	db *gorm.DB

	field.RelationField
}

func (a userHasOneTrafficPlan) Where(conds ...field.Expr) *userHasOneTrafficPlan {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasOneTrafficPlan) WithContext(ctx context.Context) *userHasOneTrafficPlan {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasOneTrafficPlan) Session(session *gorm.Session) *userHasOneTrafficPlan {
...
func (a userHasOneTrafficPlanTx) Find() (result *entity.TrafficPlan, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasOneTrafficPlanTx) Append(values ...*entity.TrafficPlan) (err error) {
...
func (a userHasOneTrafficPlanTx) Replace(values ...*entity.TrafficPlan) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasOneTrafficPlanTx) Delete(values ...*entity.TrafficPlan) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasOneTrafficPlanTx) Clear() error {
	return a.tx.Clear()
}
